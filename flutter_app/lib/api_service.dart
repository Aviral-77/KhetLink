...
class ApiService {
  static const String baseUrl = "https://geoserver.bluehawk.ai:8045/gee";
  ...
  static Future<Map<String, dynamic>> fetchNdviSeries(List coords) async {
    final url = Uri.parse('$baseUrl/mrv/timeseries');
  ...
  static Future<Map<String, dynamic>> calculateCarbon(List coords) async {
    final url = Uri.parse('$baseUrl/calculate_carbon');
  ...
  static Future<List<Map<String, dynamic>>> history({int limit = 20}) async {
    final url = Uri.parse('$baseUrl/mrv/history?limit=$limit');
  ...
  static Future<Uint8List> fetchReport(int id) async {
    final url = Uri.parse('$baseUrl/mrv/report/$id');
    final resp = await client.get(url);
